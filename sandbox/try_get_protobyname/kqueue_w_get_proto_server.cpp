...
int main()
{
    ...
    printf("proptocol details:\nProtocol Alias: %s\nProtocol name:   %s\nProtocol Number: %d\n", protocol_entry->p_aliases[0], protocol_entry->p_name, protocol_entry->p_proto);
    protocol_number = protocol_entry->p_proto;
    client_address_len =sizeof(client_address);
    server_socket = socket(AF_INET, SOCK_STREAM, protocol_number);
    if (server_socket < 0)
    {
        perror("socket error");
        return (-1);
